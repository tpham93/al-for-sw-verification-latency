...
def make_synthetic_with_changes(random_state, lengths, n_classes, n_center_per_class, n_features):
    ...
    for i, l in enumerate(lengths):
        X, y_center = make_blobs(n_samples=l, n_features=n_features, shuffle=True, centers=n_classes*n_center_per_class, random_state=i)
...
def get_pickled_dataset(dataset_name):
    with open('pkl_datasets/'+dataset_name+'.pkl', 'rb') as f:
        data = pkl.load(f)
...
def load_raw_dataset_by_name(random_seed, name):
    ...
    # synthetic dataset
    dataset_generation_functions['D0'] = lambda random_state: make_synthetic_with_changes(random_state, [2000, 2000], 2, 10, 2)
    dataset_generation_functions['D1'] = lambda random_state: make_synthetic_with_changes(random_state, [2000, 2000], 3, 10, 2)
    dataset_generation_functions['D2'] = lambda random_state: make_synthetic_with_changes(random_state, [2000, 2000], 4, 10, 2)
    
    # static dataset
    dataset_generation_functions['D3'] = lambda random_state: make_static_with_change(random_state, "phoneme")
    dataset_generation_functions['D4'] = lambda random_state: make_static_with_change(random_state, 'mfeat-morphological')
...
def get_dataset_by_name(dataset_random_seed, evalset_random_seed, name):
    ...
    eval_mask = create_eval_mask(evalset_random_seed, len(X), 0.2)
